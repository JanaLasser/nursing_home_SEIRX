...
def calculate_distribution_difference(sim_data, outbreak, emp_agent_numbers,
                                        observed_outbreaks, comp_period):
    '''
    Calculates the difference between the expected number of infected, given by
    empirically observed outbreak data in Austrian nursing homes and the number 
    of infected in a given simulation.
    ...
    --------
    distance : float
        Sum of squares of the difference between the number of total infected
        agents in the simulation and the given empirically observed outbreak.
    '''
    ...
    sim_data['cumulative_I_total'] = sim_data['cumulative_I_employee'] + \
            sim_data['cumulative_I_resident']
    # normalise the cumulative number of infected with the total number of
    # employees [residents] in the simulation
    sim_data['cumulative_I_total'] = sim_data['cumulative_I_total'] / \
                (N_sim_employees + N_sim_residents)
    ...
    emp_data['cumulative_I_total'] = emp_data['cumulative_I_employee'] + \
            emp_data['cumulative_I_resident']
    # normalisation
    emp_data['cumulative_I_total'] = emp_data['cumulative_I_total'] / \
            (N_emp_employees + N_emp_residents)

    # truncate both vectors to have the same length, do not use more than the
    # first comp_period days to compare outbreaks.
    min_T = min(len(emp_data['cumulative_I_total']),\
                len(sim_data['cumulative_I_total']))
    if min_T > comp_period: min_T = comp_period
    sim_data = sim_data[0:min_T]
    emp_data = emp_data[0:min_T]
    
    # the error metric is the sum of the squared distances between the two
    # cumulative distributions
    diff = (emp_data['cumulative_I_total'] - sim_data['cumulative_I_total']) ** 2
    distance = diff.sum()
    
    return distance


def calculate_resident_ratio_difference(sim_data, outbreak, observed_outbreaks,
                                        comp_period):
    '''
    Calculates the difference between the expected ratio of infected residents,
    given by empirically observed outbreak data in Austrian nursing homes and 
    the ratio of infected residents in a given simulation.
    ...
    sim_data['cumulative_I_resident'] = sim_data['E_resident'] + \
            sim_data['I_resident'] + sim_data['R_resident']
    sim_data['I_resident_ratio'] = sim_data['cumulative_I_resident'] / \
        (sim_data['cumulative_I_resident'] + sim_data['cumulative_I_employee'])
        
    # ********* empirical outbreak data ********* 
    emp_data = observed_outbreaks[observed_outbreaks['outbreak'] == outbreak]\
        .copy()
    emp_data['I_resident_ratio'] = emp_data['cumulative_I_resident'] / \
        (emp_data['cumulative_I_resident'] + emp_data['cumulative_I_employee'])

    # truncate both vectors to have the same length, do not use more than the
    # first comp_period days to compare outbreaks.
    min_T = min(len(emp_data['I_resident_ratio']),\
                len(sim_data['I_resident_ratio']))
    if min_T > comp_period: min_T = comp_period
    sim_data = sim_data[0:min_T]
    emp_data = emp_data[0:min_T]
    ...
    # the error metric is the sum of the squared distances between the two
    # cumulative distributions
    diff = (emp_data['I_resident_ratio'] - sim_data['I_resident_ratio']) ** 2
    distance = diff.sum()
